...
class RentalOrderController {
  ...
  pickup = catchAsync(async (req, res) => {
    const order = await roService.pickup(req.params.id, req.body, req.user);
    res.status(200).json(new ApiResponse(200, order, 'Vehicle picked up successfully'));
  });

  returnVehicle = catchAsync(async (req, res) => {
    const order = await roService.returnVehicle(req.params.id, req.body, req.user);
    res.status(200).json(new ApiResponse(200, order, 'Vehicle returned and inspected successfully'));
  });
